...
public class ServicoMidi implements Receiver {
    ...
    public void send(MidiMessage message, long timeStamp) {
        if (message instanceof ShortMessage) {
            ...
            // Converte o valor MIDI para o nosso objeto de modelo
            Nota notaCorrespondente = Nota.fromMidiValue(valorMidi);

            if (sm.getCommand() == ShortMessage.NOTE_ON && velocidade > 0) {
                // ... (lógica para registrar o tempo de início) ...

                // Notifica o início enviando o objeto Nota
                if (onNotaRecebidaCallback != null) {
                    onNotaRecebidaCallback.accept(notaCorrespondente);
                }
            } else if (sm.getCommand() == ShortMessage.NOTE_OFF || (sm.getCommand() == ShortMessage.NOTE_ON && velocidade == 0)) {
                // ... (lógica para calcular a duração) ...
                long tempoInicio = notasAtivas.remove(valorMidi);
                long tempoFim = System.currentTimeMillis();

                // 3. CALCULA A DURAÇÃO (a variável que estava faltando).
                long duracao = tempoFim - tempoInicio;

                // Notifica o fim enviando o objeto Nota e a duração
                if (onNotaFinalizadaCallback != null) {
                    onNotaFinalizadaCallback.accept(notaCorrespondente, duracao);
                }
            }
        }
    }
